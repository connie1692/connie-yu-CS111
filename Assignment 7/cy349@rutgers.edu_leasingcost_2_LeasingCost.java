public class LeasingCost {
    ...
    {
        String name = description.substring(description.indexOf("name") + 5, description.length());
        name = name.substring(0, name.indexOf(";"));
       
        String fuel = description.substring(description.indexOf("type") + 5, description.length());
        fuel = fuel.substring(0, fuel.indexOf(";"));
       
        String mpu = description.substring(description.indexOf("mile/unit") + 10, description.length());
        mpu = mpu.substring(0, mpu.indexOf(";"));
        double mpu1 = Double.parseDouble(mpu);
        String s = "gas";
        ...
        {
            String charger = description.substring(description.indexOf("charger") + 8, description.length());
            charger = charger.substring(0, charger.indexOf(";"));
            double charger1 = Double.parseDouble(charger);
           
            f = new Fuel(mpu1, charger1);  
        }

        String d = description.substring(description.indexOf("due") + 4, description.length());
        d = d.substring(0, d.indexOf(";"));
        double due = Double.parseDouble(d);
 
        String l = description.substring(description.indexOf("length") + 7, description.length());
        l = l.substring(0, l.indexOf(";"));
        int length = Integer.parseInt(l);
 
        String m = description.substring(description.indexOf("monthly") + 8, description.length());
        m = m.substring(0, m.indexOf(";"));
        double monthly = Double.parseDouble(m);
 
        String ma = description.substring(description.indexOf("allowance") + 10, description.length());
        ma = ma.substring(0, ma.indexOf(";"));
        int allowance = Integer.parseInt(ma);
 
        Lease Lease1 = new Lease(due, length, monthly, allowance);
        Vehicle car = new Vehicle(name, f, Lease1);
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice )
    {
        ...
        {
            double CurPrice = gasPrice;
            double numberOfMonths = v.getLease().getLeaseLength();
            ...
            {
                co2PerUnit = 0.453;
                CurPrice = electricityPrice;
            }
            
            double Co2Cost = computeCO2(numberOfMonths, usage, mileageAllowance, co2PerUnit);
            double FuelCost = computeFuelCost(numberOfMonths, usage, mileageAllowance, CurPrice);
            double TotalCost = computeLeaseCost(dueAtSigning, (int) numberOfMonths, monthlyCost);
            TotalCost += FuelCost;
            
            if (v.getFuel().getType() == 2)
            {
                TotalCost += v.getFuel().getCharger();
            }
            v.setCO2Emission(Co2Cost);
